...
export default async function handler(req, res) {
    ...
    try {
        ...
        // 🔒 JIKA DIBUKA DI BROWSER DENGAN METODE GET (PROTECTED PAGE)
        if (req.method === 'GET' && !req.query.action) {
            ...
            return res.status(200).send(`
                ...
                <head>
                    ...
                    <style>
                        body { background-color: #0f172a; color: #f8fafc; font-family: sans-serif; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; padding: 20px; }
                        .card { background-color: #1e293b; border: 1px solid #334155; border-radius: 16px; padding: 40px 30px; text-align: center; max-width: 420px; width: 100%; box-shadow: 0 10px 25px rgba(0,0,0,0.5); }
                        .icon-container { width: 80px; height: 80px; background: rgba(239, 68, 68, 0.1); border: 1px solid #ef4444; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px auto; color: #ef4444; }
                        h1 { font-size: 1.4rem; color: #ef4444; margin-bottom: 10px; font-weight: 700; }
                        p { font-size: 0.9rem; color: #94a3b8; line-height: 1.5; margin-bottom: 20px; }
                        .badge { background: #0f172a; border: 1px solid #334155; color: #38bdf8; padding: 8px 14px; border-radius: 8px; font-family: monospace; font-size: 0.85rem; display: inline-block; }
                    </style>
                </head>
                <body>
                    <div class="card">
                        ...
                        <p>Akses via Browser (GET) ditolak. Endpoint ini khusus digunakan untuk validasi API Key menggunakan metode <strong>POST</strong>.</p>
        ...
        // --- ACTION: CREATE KEY (CUSTOM ATAU RANDOM) ---
        if (action === 'create') {
            const { label, days, custom_key } = req.body || {};
            ...
            // Jika custom key tidak diisi, buat Random Key (vdb_...)
            if (!keyValue) {
            ...
            if (checkExist.length > 0) {
                return res.status(400).json({
                    status: false,
                    message: "Key ini sudah terpakai di database! Gunakan key lain."
                });
            }
            ...
            return res.status(200).json({ status: true, message: "API Key berhasil tersimpan!", data: result[0] });
        }
        ...
        // --- VALIDASI VIA METODE POST (HTTP INJECTOR / GAME GUARDIAN) ---
        if (req.method === 'POST') {
            ...
            if (!apiKey) {
                return res.status(400).json({
                    status: false,
                    valid: false,
                    message: "Payload JSON 'key' wajib diisi!"
                });
            }
